...
Graphics.initialize = function (width, height, type) {
    ...
    this._stretchEnabled = true;//this._defaultStretchMode();強制打開就行自在如意了
...
Graphics.resizeGame = function (width, height) {
    // 更新內部解析度
    //this._width = width;
    //this._height = height;

    // 呼叫內建的 resize 更新 renderer 與 DOM
...
Graphics._updateRealScale = function () {
    if (this._stretchEnabled) {
        var h = window.innerWidth*0.98 / this._width;
        var v = window.innerHeight*0.98 / this._height;
        if (h >= 1 && h - 0.01 <= 1) h = 1;
        if (v >= 1 && v - 0.01 <= 1) v = 1;
